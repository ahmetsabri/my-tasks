            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('task.update', $task) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input value="{{ old('title', $task->title) }}" type="text" placeholder="Title"
                            name="title" class="input input-bordered w-full max-w-xs" />

                        @error('title')
                            <p class="text-red-600">{{ $message }}</p>
                        @enderror
                        <div class="w-full my-3">
                            <input value="{{ old('due_date', $task->due_date) }}" type="date" placeholder="Due Date"
                        ...
                        <div class="w-full my-3">
                            ...
                            <select class="select select-bordered w-full max-w-xs" name="status">
                                ...
                                @foreach ($statuses as $status)
                                    <option @selected($status->value == $task->status) value="{{ $status->value }}">
                                        {{ $status->value }}</option>
                                @endforeach
                            </select>
                            @error('status')
